...
public class BindingCardActivity extends BaseActivity implements View.OnClickListener {
    ...
    private void initUI() {
        ...
        checkHintSize(et_name,"请输入收款人的姓名");
        //银行卡号
        et_number = (EditText) findViewById(R.id.edit_binding_number_card);
        checkHintSize(et_number,"请输入收款人的卡号");
        //银行账号所属的银行
        et_type = (EditText) findViewById(R.id.edit_number_type_card);
        checkHintSize(et_type,"请输入卡号相对的银行");
        ...
        if (type == 0) { //当type为0时，则处理绑定银行卡事件
            setCommonHeader(getString(R.string.binging_bank));
            ...
            setCommonHeader(getString(R.string.unbinding_bank));
        ...
        switch (v.getId()) {
            ...
            case R.id.btn_isBinding:
                if (type == 0) {  //绑定
                    ...
                    String type = et_type.getText().toString();  //所属银行
                    HashMap<String, String> params = new HashMap<String, String>();
                    params.put("key", myApplication.getLoginKey());
                    params.put("name", name);
                    params.put("account", number);
                    params.put("bankname", type);
                    params.put("type", "2");
                        ...
                        public void dataLoaded(ResponseData data) {
                            ...
                            if (data.getCode() == HttpStatus.SC_OK) {
                                try {
                                    ...
                                    Toast.makeText(BindingCardActivity.this, msg, Toast.LENGTH_SHORT).show();
                                    BindingCardActivity.this.finish();
                                } catch (Exception e) {
                                    Toast.makeText(BindingCardActivity.this, "绑定失败", Toast.LENGTH_SHORT).show();
                                }
                            }
                        }
                    });
