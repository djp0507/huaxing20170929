...
import cn.hx.hn.android.bean.ReturnBean;
import cn.hx.hn.android.bean.ReturnPayDetails;
...
public class OrderExchangeControlDetailsActivity extends BaseActivity {
    ...
    if(type.equals("money")){
       url=myApplication.getpath()+Constants.URL_ORDER_EXCHANGE_MONEY_DETAILS+"&key="+myApplication.getLoginKey()+"&refund_id="+id;
       ...
       url=myApplication.getpath()+Constants.URL_ORDER_EXCHANGE_GOODS_DETAILS+"&key="+myApplication.getLoginKey()+"&return_id="+id;
        RemoteDataHandler.asyncDataStringGet(url,new RemoteDataHandler.Callback() {
            @Override
            public void dataLoaded(ResponseData data) {
                String json=data.getJson();
